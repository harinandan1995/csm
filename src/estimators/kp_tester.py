...
class KPTransferTester(ITester):
    ...
    def _batch_call(self, step, batch_data):
        ...
        tar_kp = self._find_target_kps(src_kp, src_pred_out['uv'], tar_pred_out['uv'])
        tar_kp = torch.cat((tar_kp, src_kp[:, :, 2:].to(torch.int64)), dim=2)

        self._add_to_summaries(self._convert_to_int_indices(src_kp), tar_kp, src_img, tar_img)
    ...
    def _find_target_kps(self, src_kp, src_uv, tar_uv):

        batch_size = src_uv.size(0)
        height = src_uv.size(1)
        width = src_uv.size(2)
        ...
        kp_dist = torch.mean(kp_dist, dim=-1)

        tar_kp = unravel_index(torch.argmax(kp_dist, 2), (1, height, width, 1))
    ...
    def _add_to_summaries(self, src_kp, tar_kp, src_img, tar_img):

        self.summary_writer.add_images('src', draw_key_points(src_img, src_kp, self.key_point_colors))
        self.summary_writer.add_images('tar', draw_key_points(tar_img, tar_kp, self.key_point_colors))
